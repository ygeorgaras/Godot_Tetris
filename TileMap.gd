...
#This will reset all variables from the previous piece
func create_piece():
	steps = [0,0,0]
	cur_pos = start_pos
...
func move_piece(dir):
	if can_move(dir): 
		clear_piece()
		cur_pos += dir #moves the piece down one according to the cur_pos
		draw_piece(active_piece, cur_pos, piece_atlas)
	else:
...
func can_move(dir):
	#checks if there is space to move
	var cm = true
	for i in active_piece:
...
func can_rotate():
	var cr = true
	var tmp_rotation_index = (rotation_index + 1) % 4
	for i in piece_type[tmp_rotation_index]:
	...
	while row > 0:
		...
		for i in range(COLS):
			if not is_free(Vector2i(i+1, row)):
				count += 1
		#if row is full than erase it
		if count == COLS:
			shift_rows(row)
			score += REWARD
			$HUD.get_node("ScoreLabel").text = "Score: " + str(score)
			speed += ACCEL#Effects the speed for each row cleared
		else:
			row -= 1
